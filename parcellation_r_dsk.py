...
# setup nifti masker
masker = NiftiLabelsMasker(labels_img=desikan_atlas, low_pass=0.1, high_pass=0.08, memory=memory, verbose=0)
masker.fit()
...
time_series_file_desikan = os.path.join(out_dir, "TS_rest_dsk_v2_{}.npy".format(timestamp))
mat_file_desikan = time_series_file_desikan.replace(".npy", ".mat")
...
if len(unique_shapes) == 1:
	...
	scipy.io.savemat(mat_file_desikan, mdict={'time_series': time_series_all_np, 'subject_ids': subject_ids_mat}) # export as mat file
	np.save(time_series_file_desikan, time_series_all_np) # save as npy file
	print("Time series saved to", time_series_file_desikan)
	print("MAT file saved to", mat_file_desikan)
	print("Time series shape", time_series_all_np.shape)
else:
	ragged_file = time_series_file_desikan.replace(".npy", "_ragged.npy")
	...
	scipy.io.savemat(mat_file_desikan, mdict={'time_series': time_series_ragged, 'subject_ids': subject_ids_mat})
...
# subject_ids_file = os.path.join(out_dir, "subject_ids_rest_desikan_{}.csv".format(timestamp))
subject_ids_df = pd.DataFrame(kept_subject_ids, columns=["subject_id"])
subject_ids_file = os.path.join(out_dir, "subID_rest_dsk_v2_{}.csv".format(timestamp))
...
if error_files:
	...
	error_file = os.path.join(out_dir, "errsubID_rest_dsk_v2_{}.csv".format(timestamp))
