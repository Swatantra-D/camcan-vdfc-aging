...
base_data_dir = r"/scratch/data/p25ai0202/swat/cam_data/rst_dswau"  # the base data directory
...
scf_atlas = os.path.join(roi_dir, "Schaefer2018_400Parcels_7Networks_order_FSLMNI152_2mm.nii.gz") # get schaefer atlas
...
file_pattern = os.path.join(data_dir, "dswausub-CC*.nii*") # define file pattern
...
# setup nifti masker
masker = NiftiLabelsMasker(labels_img=scf_atlas, memory=memory, verbose=0)
...
time_series_file_scf = os.path.join(out_dir, "TS_rest_scf_{}.npy".format(timestamp))
mat_file_scf = time_series_file_scf.replace(".npy", ".mat")
...
if len(unique_shapes) == 1:
	...
	scipy.io.savemat(mat_file_scf, mdict={'time_series': time_series_all_np, 'subject_ids': subject_ids_mat}) # export as mat file
	np.save(time_series_file_scf, time_series_all_np) # save as npy file
	print("Time series saved to", time_series_file_scf)
	print("MAT file saved to", mat_file_scf)
	print("Time series shape", time_series_all_np.shape)
else:
	ragged_file = time_series_file_scf.replace(".npy", "_ragged.npy")
	...
	scipy.io.savemat(mat_file_scf, mdict={'time_series': time_series_ragged, 'subject_ids': subject_ids_mat})
...
# subject_ids_file = os.path.join(out_dir, "subject_ids_movie_scf_all_denoised_{}.csv".format(timestamp))
subject_ids_df = pd.DataFrame(kept_subject_ids, columns=["subject_id"])
subject_ids_file = os.path.join(out_dir, "sub_ids_rest_scf_{}.csv".format(timestamp))
...
if error_files:
	...
	error_file = os.path.join(out_dir, "error_sub_rest_scf_{}.csv".format(timestamp))
