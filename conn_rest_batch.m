...
data_folder ='/scratch/data/p25ai0202/swat/cam_data/left';
cd(data_folder);
...
cwd=pwd;
FUNCTIONAL_FILE=cellstr(conn_dir('sub-*_epi_rest.nii.gz'));
...
for batch_idx = 1:num_batches
    ...
    batch.Setup.conditions.names = {'rest'}; % condition name (can be anything, but must be consistent across subjects)
    for ncond = 1
        for nsub = 1:NSUBJECTS_BATCH
            for nses = 1:nsessions
