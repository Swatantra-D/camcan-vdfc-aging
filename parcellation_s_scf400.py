...
base_data_dir = r"/scratch/data/p25ai0202/swat/cam_data/smt_dswau"  # the base data directory
...
time_series_file_scf= os.path.join(out_dir, "TS_smt_scf400_{}.npy".format(timestamp))
...
# subject_ids_file = os.path.join(out_dir, "subID_smt_dsk_{}.csv".format(timestamp))
subject_ids_df = pd.DataFrame(kept_subject_ids, columns=["subject_id"])
subject_ids_file = os.path.join(out_dir, "subID_smt_scf400_{}.csv".format(timestamp))
...
if error_files:
	...
	error_file = os.path.join(out_dir, "error_subID_smt_scf400_{}.csv".format(timestamp))
